Welcome to Open Orchestra's documentation!
==========================================

.. _Open Orchestra website: http://www.open-orchestra.com/
.. _Open Orchestra GitHub: http://www.github.com/open-orchestra/
.. _Change log: https://github.com/open-orchestra/open-orchestra/tree/master/app/Resources/doc/changeLog

The main documentation is organized into a couple sections:

* :ref:`intro`
* :ref:`user-guide`
* :ref:`developer-guide`
* :ref:`hosting-guide`
* :ref:`misc`

Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`

.. _intro:

.. toctree::
   :maxdepth: 2
   :caption: Introduction

   key_concepts

.. _user-guide:

.. toctree::
   :maxdepth: 2
   :caption: User guide

   user_guide/websites_creation
   user_guide/node
   user_guide/node_configuration
   user_guide/content_type
   user_guide/content
   user_guide/content_display
   user_guide/block_list
   user_guide/media
   user_guide/global_page_blocks
   user_guide/keyword_management
   user_guide/user
   user_guide/role
   user_guide/workflow

.. _developer-guide:

.. toctree::
   :maxdepth: 2
   :caption: Developer guide

   developer_guide/install
   developer_guide/install_with_vagrant
   developer_guide/install_with_vagrant_for_contributors
   developer_guide/dependancy_diagram
   developer_guide/bundle_configuration
   developer_guide/custom_fixtures_load
   developer_guide/block_creation
   developer_guide/block_parameter
   developer_guide/block_display
   developer_guide/block_override
   developer_guide/navigation_panel
   developer_guide/field_type
   developer_guide/entity_list_ajax_pagination
   developer_guide/specific_backbone_view
   developer_guide/backbone_routing
   developer_guide/manage_assets
   developer_guide/deploy_assets
   developer_guide/multi_panel
   developer_guide/multi_device
   developer_guide/esi
   developer_guide/themes
   developer_guide/twig_extensions
   developer_guide/media_gaufrette
   developer_guide/media_standalone_display
   developer_guide/smart_admin
   developer_guide/events
   developer_guide/api
   developer_guide/api_transformer
   developer_guide/api_group_context
   developer_guide/routing
   developer_guide/extend_document
   developer_guide/content_add_field_listener
   developer_guide/custom_content_attribute_display
   developer_guide/prevent_document_suppression_when_embed
   developer_guide/dashboard_widgets
   developer_guide/error_pages
   developer_guide/robots
   developer_guide/sitemap
   developer_guide/bbcode
   developer_guide/assets_bower_npm
   developer_guide/other_database_type
   developer_guide/elasticasearch_indexation

.. _hosting-guide:

.. toctree::
   :maxdepth: 2
   :caption: Hosting guide

   hosting_guide/requirements
   hosting_guide/configuration
   hosting_guide/deploy
   hosting_guide/server_provisioning
   hosting_guide/logs
   hosting_guide/migration
   hosting_guide/site_creation

.. _misc:

.. toctree::
   :maxdepth: 2
   :caption: Miscellaneous

   quality_summary
